...
void soldynamique(int n,point poly[],corde solution[]){
	...
	for(i = 3; i < n; i++){
		for(j = 0; j < n; j++){
			...
			c.p2 = poly[j+i];
			c.lon = coutcorde2(c);
			//TODO:Trouver le min des sous solutions possibles
			//Remplissage de la diagonale TODO
			tab_sous_poly[j][i-2].somme_corde = c.lon;
			tab_sous_poly[j][i-2].nb_corde = i - 1; 
			tab_sous_poly[j][i-2].tab_corde[0] = c ;
			//TODO:ATTENTION pour les deux dernières valeurs faire un SI comme plus haut
